  ...
  <body>
        ...
        <div class="content-wrapper">
          ...
          <div class="table-container">
            ...
            <table class="data-table" id="suspensionsTable">
              ...
              <tbody>
                @foreach($data as $suspension)
                <tr>
                  <td>{{ $suspension->type }}</td>
                  <td>{{ \App\Models\Motorcycle::where('front_suspension_id', $suspension->id)->orWhere('rear_suspension_id', $suspension->id)->count() }}</td>
                  <td>
    ...
    <script>
      function searchSuspensions() {
        ...
        const table = document.getElementById('suspensionsTable');
        const rows = table.getElementsByTagName('tr');
        
        for (let i = 1; i < rows.length; i++) {
          const suspensionType = rows[i].getElementsByTagName('td')[0].textContent.toLowerCase();
          if (suspensionType.includes(input)) {
      ...
      function confirmDelete(event, suspensionType) {
        event.preventDefault();
        if (confirm(`Tem certeza que deseja excluir o tipo de suspensão "${suspensionType}"?`)) {
